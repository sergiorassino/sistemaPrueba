    ...
    @if (! empty($whatsappWaBanner))
        <div class="se-soft-card border border-accent-200 bg-white px-4 py-4 text-sm text-neutral-800 shadow-sm">
            ...
            <p class="mt-1 text-xs leading-relaxed text-neutral-600">
                Abra cada enlace para cargar el mensaje en WhatsApp Web o en la app; luego confirme el envío en WhatsApp.
            </p>
                                        ...
                                        @if ($msg->destinatarios->count())
                                            <div class="flex flex-wrap items-center justify-end gap-1">
                                                @foreach ($msg->destinatarios->first()?->envios ?? [] as $envio)
                                                    @if ($envio->medio === 'whatsapp'
                                                        && $envio->estado === 'enviado'
                                                        && str_starts_with((string) $envio->proveedor_msgid, 'https://wa.me/'))
                                                        <a href="{{ $envio->proveedor_msgid }}"
                                                           target="_blank"
                                                           rel="noopener noreferrer"
                                                           title="WhatsApp: abrir conversación"
                                                           @class([
                                                               'inline-flex text-[10px] no-underline',
                                                               'text-primary-700 hover:text-primary-800' => $esPlataforma,
                                                               'text-white/90 hover:text-white' => ! $esPlataforma,
                                                           ])>
