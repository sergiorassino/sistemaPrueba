                ...
                <table class="min-w-full divide-y divide-accent-200 text-sm">
                    ...
                    <tbody class="divide-y divide-accent-100 bg-white">
                        @forelse ($matriculas as $mat)
                            @php
                                $leg = $mat->legajo;
                                $ap = trim((string) ($leg?->apellido ?? ''));
                                $nom = trim((string) ($leg?->nombre ?? ''));
                                $linea = $ap === '' && $nom === '' ? '—' : trim($ap.(($ap !== '' || $nom !== '') ? ', ' : '').$nom);
                            @endphp
                            <tr class="hover:bg-accent-50/60">
                                <td class="px-5 py-3 font-medium text-neutral-800">{{ $linea }}</td>
                                <td class="px-5 py-3 text-neutral-600">{{ trim((string) ($leg?->dni ?? '')) !== '' ? $leg->dni : '—' }}</td>
                                <td class="px-5 py-3 text-right">
                                    <a href="{{ route('calificacionesSecundario.consulta.pdf', ['matricula' => $mat->id]) }}"
                                       target="_blank"
                                       rel="noopener noreferrer"
                                       class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-accent-200 bg-white px-3 py-2 text-xs font-semibold text-primary-700 shadow-sm transition hover:border-primary-300 hover:bg-accent-50">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
